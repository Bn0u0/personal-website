(()=>{
  ...
  if(detail){
    ...
    // Lenis owns the page scroll. The project detail is a native, independent
    // scroll surface and must never have its wheel/touch events hijacked.
  ...
  function renderEvidence(key){
    ...
    if(detailMedia){
      ...
      if(!rail){
        ...
        rail.className='detail-evidence-rail';
        detailMedia.appendChild(rail);
      }
    ...
    const journey=story?.journey?.length?`
      <section class="detail-journey" aria-label="PicNest evolution">
        <div class="detail-section-head"><span>${lang()==='zh'?'歷程 / TURNING POINTS':'JOURNEY / TURNING POINTS'}</span><span>${String(story.journey.length).padStart(2,'0')} / EVOLUTION</span></div>
        <div class="detail-journey__intro"><h3>${esc(story.journeyTitle)}</h3><p>${esc(story.journeyIntro)}</p></div>
        <div class="detail-journey__list">${story.journey.map((step,i)=>`<article class="detail-journey-step"><div><span>${String(i+1).padStart(2,'0')}</span><small>${esc(step[0])}</small></div><div><strong>${esc(step[1])}</strong><p>${esc(step[2])}</p></div></article>`).join('')}</div>
        <div class="detail-journey__reflection"><span>REFLECTION / 001</span><div><strong>${esc(story.reflectionTitle)}</strong><p>${esc(story.reflection)}</p></div></div>
      </section>`:'';
  ...
  // site.js opens the modal on the row itself. This document-level listener runs
  // after that handler, so the rich case-study markup always wins over the
  // one-line fallback that site.js initially writes into .detail-below-copy.
  ...
  addEventListener('popstate',()=>{
    if(uiClosing)return;
    if(detail?.classList.contains('is-open')&&location.pathname==='/')detailClose?.click();
  });
  ...
  const langObserver=new MutationObserver(()=>{
    syncRows();
    if(detail?.classList.contains('is-open'))renderEvidence(currentKey);
  });
  ...
  caseDataReady.then(()=>{
    if(detail?.classList.contains('is-open')&&currentKey==='picnest')renderEvidence(currentKey);
  });
